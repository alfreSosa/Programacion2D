...
{
    SCREEN.Open(800, 600, false);
    Scene *escena1 = new Scene(nullptr);
    ...
    Sprite *generador = nullptr;
    Emitter *emisor = nullptr;
    if (star)
    {
...
void practica5()
{
    ...
    while (SCREEN.IsOpened() && !SCREEN.KeyPressed(GLFW_KEY_ESCAPE))
    {
        ...
        for (unsigned int i = 0; i < 5; i++)
        {
            ...
            sprites[i]->Update(SCREEN.ElapsedTime(), nullptr);
            sprites[i]->Render();
        }
        SCREEN.Refresh();
...
void practica1()
{
    ...
    while (SCREEN.IsOpened() && !SCREEN.KeyPressed(GLFW_KEY_ESCAPE))
    {
        ...
        glRectd(x_midle, y_midle, x_midle + rectCenterTam, y_midle + rectCenterTam);
        ...
        //    ((rgba >> 8) & 0xff) / 255.0f,
        //    (rgba & 0xff) / 255.0f);
        //glVertex3f(x_midle, y_midle, 0);
        //glVertex3f(x_midle + rectCenterTam, y_midle, 0);
        //glVertex3f(x_midle + rectCenterTam, y_midle + rectCenterTam, 0);
        //glVertex3f(x_midle, y_midle + rectCenterTam, 0);
        //glEnd();
        //glColor4f(1, 1, 1, 1);
